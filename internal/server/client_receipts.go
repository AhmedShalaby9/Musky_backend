...
func loadClientLedger(ctx context.Context, db *gorm.DB, tenant, clientID uint64) (model.Client, []model.LedgerEntry, error) {
	...
	opening := tx.Table("clients c").Select("'opening' AS kind, NULL AS ref_id, NULL AS return_id, NULL AS invoice_number, '' AS document_type, CASE WHEN c.opening_balance_type='payable' THEN -c.opening_balance_minor ELSE c.opening_balance_minor END AS delta_minor, NULL AS method, '' AS notes, c.created_at AS at").Where("c.tenant_id = ? AND c.id = ?", tenant, clientID)
	ledger := tx.Table("client_ledger l").Select("l.kind, i.id AS ref_id, l.return_id, i.number AS invoice_number, i.document_type, l.amount_minor AS delta_minor, NULL AS method, CASE WHEN l.kind='return' THEN COALESCE(r.reason,'') ELSE COALESCE(i.void_reason,'') END AS notes, CASE WHEN l.kind='return' THEN r.created_at WHEN l.kind IN ('void','purchase_void') THEN COALESCE(i.voided_at,i.posted_at,i.created_at) ELSE COALESCE(i.posted_at,i.created_at) END AS at").Joins("JOIN invoices i ON i.id=l.invoice_id AND i.tenant_id=l.tenant_id").Joins("LEFT JOIN invoice_returns r ON r.tenant_id=l.tenant_id AND r.id=l.return_id").Where("l.tenant_id = ? AND l.client_id = ?", tenant, clientID)
	payments := tx.Table("invoice_payments p").Select("'invoice_payment' AS kind, p.invoice_id AS ref_id, NULL AS return_id, i.number AS invoice_number, i.document_type, CASE WHEN i.document_type='purchase' THEN p.amount_minor ELSE -p.amount_minor END AS delta_minor, p.method, p.notes, p.paid_at AS at").Joins("JOIN invoices i ON i.id=p.invoice_id AND i.tenant_id=p.tenant_id").Where("p.tenant_id = ? AND p.client_id = ?", tenant, clientID)
	receipts := tx.Model(&model.ClientReceipt{}).Select("IF(reversal_of_id IS NULL,IF(direction='out','client_payment','receipt'), 'reversal') AS kind, id AS ref_id, NULL AS return_id, NULL AS invoice_number, '' AS document_type, IF(reversal_of_id IS NULL,IF(direction='out',amount_minor,-amount_minor),IF(direction='out',-amount_minor,amount_minor)) AS delta_minor, method, notes, received_at AS at").Where("tenant_id = ? AND client_id = ?", tenant, clientID)
