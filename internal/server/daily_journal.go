...
func (a *API) dailyJournal(c *gin.Context) {
	...
	payments := db.Table("invoice_payments p").Select("p.id, 'invoice_payment' AS kind, p.invoice_id, i.number AS invoice_number, i.document_type, c.name AS client_name, p.amount_minor, p.method, COALESCE(p.notes,'') AS notes, p.paid_at").
	...
	receipts := db.Table("client_receipts r").Select("r.id, 'client_receipt' AS kind, NULL AS invoice_id, NULL AS invoice_number, '' AS document_type, c.name AS client_name, r.amount_minor, r.method, COALESCE(r.notes,'') AS notes, r.received_at AS paid_at").
